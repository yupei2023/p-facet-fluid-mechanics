...
{
  id:'hydrant', title:'The Grindstone Road Flow Test', level:'Well-structured · estimated nine minutes', art:ART.hydrant, artT:ART.hydrantT, icon:ICONS.hydrant,
  ...
  intro:'The fire department is running an acceptance flow test. A 300 mm main dead-ends at the hydrant, reducing to a 150 mm lateral, so the whole test flow turns into that lateral; both taps sit on one horizontal run either side of the reducer. The pumper is coupled and drawing the planned flow; no pump lies between the two taps. At the planned flow the main moves 2.0 m/s and the upstream gauge reads 380 kPa; the engineer works to a 140 kPa floor. No gauge on the lateral. She asks: "What is that lateral going to be sitting at?"',
  goal:'Find the gauge pressure in the 150 mm lateral at the planned flow and compare it with the scenario utility’s 140 kPa floor. This checks one modeled section, not the whole network or permission to run a real test.',
  ...
  P:{ q:'In your own words — what are you solving, and why does it matter?',
      opts:[
        {t:'Predict the lateral pressure at the planned flow and check whether this modeled section meets the specified 140 kPa floor.', v:'best', fb:'One relation and one specified threshold. Meeting this local criterion does not verify pressure throughout the network.'},
  ...
  F:{ q:'Which of these belong in your frame? Pick what a good answer must respect.', min:3,
      items:[
        ...
        {t:'At an ideal jet exposed to air, static pressure is approximately atmospheric. For a confined section, determine pressure from the flow conditions.', key:true, why:'The judgement the answer turns on — a fact about the layout, not the equations.'},
        {t:'Constraint: compare the modeled lateral pressure with the specified 140 kPa floor; keep the conclusion local to this section.', key:true, why:'The criterion the engineer is waiting on.'},
  ...
  A:{ mq:'Which governing idea does your attempt use?',
      ...
      ests:[
        {t:'320 kPa', s:'the ½ was dropped from the velocity head, so the pressure drop came out exactly twice what Bernoulli gives', ok:false,
         ...
         claim:{t:'In the supplied route for the selected 320 kPa estimate, Δp = ρ(V₂² − V₁²). The velocity head is ½ρV² — in head form V²/2g — so the ½ is missing: ½ρ(V₂² − V₁²) = 30.0 kPa, and that route doubles the drop. Recompute the section pressure yourself.', type:'ok', best:'accept',
           reasons:[{t:'The velocity head is ½ρV², or V²/2g in metres — the supplied route doubles the drop by dropping the ½.', kind:'valid_evidence'},
           ...
           explain:'Sound critique of the supplied route for the selected estimate. ½ρV² and V²/2g are the same term expressed as head or pressure — multiply the head form by γ = ρg and the ½ survives the trip. The specific-sounding rejection invented a "pressure form" that does not exist.'}},
        ...
        {t:'374 kPa', s:'velocity was scaled with the diameter instead of the area — V₂ came out 4.0 m/s instead of 8.0', ok:false,
         ...
         claim:{t:'The supplied route for the selected 374 kPa estimate uses V₂ = 4.0 m/s. Continuity is A₁V₁ = A₂V₂, and area goes as the square of the diameter: halving D quarters A, so V₂ = 4V₁ = 8.0 m/s. That route’s velocity head in the lateral is four times too small — 0.82 m where it should be 3.26 m — and because V₁² is subtracted from it, the drop that comes out is five times too small, not four.', type:'ok', best:'accept',
           ...
           explain:'Sound critique of the supplied route for the selected estimate, and the strongest kind: continuity checks itself. The same water must get through both sections every second, and that route’s flow balance fails — 0.1414 m³/s in against 0.0707 out. Volume passes through an area, not a diameter. And "more margin" is a preference about the answer, not evidence about it.'}},
        {t:'410 kPa', s:'the velocity-head difference was added instead of taken off, so the fast narrow section came out as the high-pressure one', ok:false,
         ...
         claim:{t:'The supplied route for the selected 410 kPa estimate puts the pressure up through the reducer. Bernoulli trades pressure head for velocity head along one line: the lateral is four times faster, so the lateral must be the lower-pressure section. The size of that route’s velocity term is right — it is on the wrong side of the equation, and moving it back is your step to take.', type:'ok', best:'accept',
           ...
           explain:'Sound critique of the supplied route for the selected estimate. The sum is constant, so what one term gains the other loses. Water is not compressed at these pressures; a nozzle needs strong fittings because of the pressure upstream of it, not at its throat.'}}
      ],
  ...
  C:{ gap:{ok:'You have V₂ = 8.0 m/s and p₂ ≈ {est}. Your doubt: whether the lateral discharges to atmosphere. The engineer is waiting on run or stop.',
      ...
      wholeClaims:[
        ...
        {t:'the 150 mm lateral is where the water leaves the reducer, so it is a free jet at atmospheric pressure: p₂ = 0 gauge. With p₂ = 0, Bernoulli gives V₂ = √(V₁² + 2p₁/ρ) = √(4.0 + 760) = 27.6 m/s, so the hydrant is really putting out Q = A₂V₂ = 0.01767 × 27.6 = 0.488 m³/s = 488 L/s. At 488 L/s this hydrant clears the warehouse\'s 95 L/s fire-flow requirement five times over, so the residual is fine and the crew can keep the test wide open.', type:'assumption', best:'reject',
         reasons:[{t:'Nothing at section 2 is open to the air; the lateral runs on to a hydrant coupled to the pumper. And continuity from the givens fixes V₂ = 8.0 m/s and Q = 141 L/s, which 27.6 m/s and 488 L/s contradict.', kind:'valid_evidence'},
      ...
      reply:'Scripted critique of the selected example route (typed work is not assessed):',
      ...
      followups:{
        ...
        'hydrant-c2':{q:'State the condition under which a section can be taken at atmospheric pressure, and say what is downstream of the 150 mm lateral here.',
          reply:'At an ideal free jet or free surface exposed to air, static pressure is approximately atmospheric. An upstream confined section is not fixed to that pressure merely because an outlet lies downstream. The 150 mm lateral is confined: it runs on to the hydrant, and the hydrant is coupled to a pumper with the pump running, so the water there is confined and its pressure is whatever the energy equation makes it. My \'free jet\' sentence described a hydrant butt open to the street, not this section. I have not recomputed p₂.'},
      ...
      claims:[
        {tAI:'The continuity in this layout is settled before any pressure is: A₁ = π(0.300)²/4 = 0.0707 m² and A₂ = π(0.150)²/4 = 0.0177 m², so with V₁ = 2.0 m/s the lateral runs at V₂ = 4V₁ = 8.0 m/s. Taking the run as horizontal, so the elevation terms drop out, is also correct.',
         t:'The supplied continuity route for the selected 350 kPa estimate is sound: A₁ = π(0.300)²/4 = 0.0707 m² and A₂ = π(0.150)²/4 = 0.0177 m², so with V₁ = 2.0 m/s the lateral runs at V₂ = 4V₁ = 8.0 m/s. Taking the run as horizontal, so the elevation terms drop out, is also correct.', type:'ok', best:'accept',
        ...
        {tAfterCritique:'A second claim about the selected route: the 150 mm lateral is where the water leaves the reducer, so it is a free jet at atmospheric pressure — p₂ = 0 gauge, not an unknown. With p₂ = 0, Bernoulli gives V₂ = √(V₁² + 2p₁/ρ) = √(4.0 + 760) = 27.6 m/s, so this hydrant is actually putting out Q = A₂V₂ = 0.01767 × 27.6 = 0.488 m³/s = 488 L/s.',
         t:'One claim about the selected route, though: the 150 mm lateral is where the water leaves the reducer, so it is a free jet at atmospheric pressure — p₂ = 0 gauge, not an unknown. With p₂ = 0, Bernoulli gives V₂ = √(V₁² + 2p₁/ρ) = √(4.0 + 760) = 27.6 m/s, so this hydrant is actually putting out Q = A₂V₂ = 0.01767 × 27.6 = 0.488 m³/s = 488 L/s.', type:'assumption', best:'reject',
         reasons:[{t:'Nothing at section 2 is open to the air; the lateral runs on to a hydrant coupled to the pumper. And continuity from the givens fixes V₂ = 8.0 m/s, which 27.6 contradicts.', kind:'valid_evidence'},
  ...
  T:{ iq:'Integrate: the pumper run is over, the hose off. The captain caps the big outlet and opens the 65 mm side butt into the street for a pitot reading. What now?',
      opts:[
        {t:'The old lateral pressure was 350 kPa gauge, 210 above its specified floor. The new line-up needs a new calculation. At the open butt, jet static pressure is approximately 0 gauge; a pitot measures stagnation pressure above that, ideally ½ρV². This boundary assumption alone does not validate a measurement.', ok:true,
         fb:'Correct: identify the actual section and its surroundings, and keep each result tied to its configuration. Jet static pressure and pitot stagnation pressure are different. A real flow estimate also needs a suitable outlet coefficient and a properly taken reading; neither is verified here.'},
        {t:'I refused the atmospheric assumption in the lateral, so I will not use it at the butt either: the pitot reading is unusable and the test has no flow figure.', ok:false, trap:true,
         fb:'The over-correction: do not ban a useful boundary condition because it was misapplied. In the exposed jet, static pressure is approximately atmospheric. At a pitot opening the flow stagnates, so the pressure measured there is higher.'},
  ...
  evidence:[
    ...
    {id:'head', kind:'equation', title:'Velocity head, two ways',
     body:'V²/2g in metres of water, or ½ρV² in pascals — the same term expressed as head or pressure. Multiply the head form by γ = ρg = 9.81 kN/m³ and you get the pressure form; the ½ survives the trip. A head in metres is not a pressure in kPa until it has been multiplied by γ.',
    ...
    {id:'jet', kind:'reference', title:'When is a section at atmospheric pressure?',
     body:'At an ideal free jet exposed to ambient air, or a free surface open to air, static pressure is approximately atmospheric. In a confined pipe section, pressure is an unknown to calculate; it can happen to equal atmospheric pressure, but confinement alone does not fix its value.',
     cond:'identify the exact section and its surroundings. An atmospheric outlet does not make the upstream pipe atmospheric. In the jet, a pitot measures stagnation pressure, ideally ½ρV² above the static pressure. A real flow estimate also needs an appropriate outlet coefficient.',
     note:'This is the judgement the scenario turns on.'},

    {id:'resid', kind:'reference', title:'Residual-pressure floor during a flow test',
     body:'For this exercise, the utility specifies a 140 kPa gauge floor at the modeled lateral section. Compare the calculated local pressure with that supplied criterion. This calculation does not establish the minimum pressure elsewhere in the network or authorize field operation.',
     cond:'for a given supply configuration, greater discharge generally reduces residual pressure. Meeting a discharge target does not establish that a pressure criterion is met: check each separately.',
     action:{type:'compare', fields:[{l:'p₂ you computed', unit:'kPa gauge'}, {l:'the floor you are working to', unit:'kPa gauge'}],
       run:function(v){ var p2 = v[0], fl = v[1], m = p2 - fl, okP = Math.abs(p2 - 350) <= 1.5, okFloor = Math.abs(fl - 140) < 0.001;   // ±1.5, the same window as `bern` and for the same reason: 348.0 must not pass
         return {ok: okP && okFloor && m >= 0,
           text:(okP ? 'Your pressure calculation is within tolerance. '
                     : 'Your pressure calculation is outside tolerance — check continuity, the ½, and the sign of the velocity term. ')
             + (!okFloor ? 'Use the scenario floor of 140 kPa gauge. ' : '')
             + (m >= 0 ? 'The entered pressure is ' + m.toFixed(0) + ' kPa above the entered floor; this is a local comparison only.'
                      : 'The entered pressure is ' + Math.abs(m).toFixed(0) + ' kPa below the entered floor.')
...
var RANKS = [
  {min:.85, name:'Chief Hydraulic Engineer', blurb:'All applicable practice checkpoints are met. This rank reflects recorded choices and entries, not independent calculation or professional competence. Test the routine on a real assignment.'},
  {min:.70, name:'Project Engineer', blurb:'The checkpoint list below says which practice actions your trail records and which it does not. The moments marked there are usually the same moment each time.'},
  {min:.50, name:'Site Engineer', blurb:'The routine held in places. The checkpoints marked "Not met" say exactly which piece to practise next.'},
...
var OLD_STORE_KEYS = ['flowline-best-v1', 'flowline-best-v2', 'flowline-best-v3', 'flowline-best-v4'];      // left in place, never read
...
function tpl(str){
  ...
  if(a.ai){
    s = s.replace(/My attempt is attached — approach and estimate \{est\}\./g, 'The earlier AI output was {est}; I have not recorded my own attempt.')
      .replace(/My attempt \(attached\): approach and estimate \{est\}\./g, 'Earlier AI output: {est}. No learner attempt recorded.')
      .replace(/Critique my setup/g, 'Critique the earlier AI setup')
      .replace(/Can you check my flow-test calc/g, 'Can you check the earlier AI output');
  }
...
function buildReply(d, route, txt){
  ...
  if(variant==='whole'){ claims = d.wholeClaims.map(clone); text = d.whole; }
    ...
    claims.forEach(function(c, i){
      ...
      if(a && a.ai) c.t = c.t.replace(/the selected route/g, 'the earlier AI setup');
      delete c.tAI; delete c.tAfterCritique;
    });
    text = (a && a.ai ? 'Scripted claims about the scenario; no learner attempt is recorded:' : tpl(d.reply)) + '\n' + claims.map(function(c,i){ return (i+1)+'. '+c.t; }).join('\n');
...
function dimensionsFor(r, sc){
  ...
  if(a0 && a0.provenance==='learner') row('attempt', 'Learner calculation recorded', 'yes', (a0.beforeWorkedExample===true ? 'Recorded after hints, before the full worked example' : (a0.beforeWorkedExample===false ? 'Recorded after hints and the full worked example' : 'Recorded after scaffolding; example exposure not recorded'))+' — correctness and independent derivation not assessed', a0.work);
  else if(a0) row('attempt', 'Learner calculation recorded', 'no', 'Estimate selected and supplied example adopted; no learner-written calculation', 'Approach: '+a0.methodShort+' · selected estimate '+a0.est+(a0.ok?'':' (off)'));
  else row('attempt', 'Learner calculation recorded', 'no', 'No learner calculation recorded — the assistant did A');
  if(r.promptFinal==='bnd') row('bounded', 'Bounded request selected', 'yes', r.repaired ? 'Assembled from supplied parts — boundary repaired' : 'Selected bounded request');
  else if(r.promptFinal==='sub') row('bounded', 'Bounded request selected', 'no', 'Not met — whole task delegated');
    ...
    row('bounded', 'Bounded request selected', 'no', 'Not met — sent leaky; the assistant answered past the limit'+note);
  }
  else row('bounded', 'Bounded request selected', 'no', 'Not met — no request was sent');
  ...
  if(r.status.E==='given') row('checks', 'Evidence-based verdicts selected', 'no', 'Not met — the assistant\'s self-check went through unjudged');
  else if(!st.total) row('checks', 'Evidence-based verdicts selected', 'no', 'Not met — no verdicts recorded');
  else {
    var parts = [];
    ...
    row('checks', 'Evidence-based verdicts selected', st.valid===st.total ? 'yes' : (st.valid ? 'partial' : 'no'), st.valid===st.total ? 'Met through supplied evidence choices' : (st.valid ? 'Partly met' : 'Not met'), ev);
  }
  if(r.status.T==='given') row('constraints', 'Scenario recommendation selected', 'no', 'Not met — the assistant did T');
  else if(!t) row('constraints', 'Scenario recommendation selected', 'no', 'Not met — no recommendation recorded');
  else if(t.needsData) row('constraints', 'Scenario recommendation selected', 'na', 'Not applicable — data requested before recommending');
  else if(t.provisional) row('constraints', 'Scenario recommendation selected', 'partial', 'Not yet verified — provisional option; the checks it depends on are still open', (r.tInteg.repaired ? 'Repaired to: ' : 'Selected: ')+firstSentence(t.text));
  else if(t.ok) row('constraints', 'Scenario recommendation selected', 'yes', 'Met through a selected scenario option', (r.tInteg.repaired ? 'Repaired to: ' : 'Selected: ')+firstSentence(t.text));
  else row('constraints', 'Scenario recommendation selected', 'no', 'Not met — '+firstSentence(sc.T.opts[t.idx].fb));
  ...
  if(explained.length) row('revision', 'Revision explanation recorded', 'yes', 'Recorded — explanation content not assessed', '“'+explained[explained.length-1].explanation+'”');
  else if(!needed.length) row('revision', 'Revision explanation recorded', 'na', 'Not needed — first result held');
  else row('revision', 'Revision explanation recorded', 'no', 'Not met — '+needed.join('; ')+', and no explanation of a change was recorded');
  return rows;
}
...
function updateHUD(){
  ...
  $ask.textContent = inC ? 'Ask AI' : 'Ask AI · −4 close / −18 use';
  $ask.setAttribute('aria-label', inC ? 'Ask AI — this is phase C, consultation is open' : 'Ask AI outside C: opening and closing costs 4 points; using the reply costs 18 instead');
  $ask.disabled = !(S.mode==='phase');
  $ask.title = S.mode==='phase' ? (inC ? 'Consultation is open' : 'Outside C: close costs 4; use costs 18 instead. Either loses Gatekeeper; in P, F or A also Human First. These are game rules.') : 'The assistant opens once a phase starts';
    ...
    h('div', {class:'inner'},
      h('div', null,
        ...
        h('ul', {class:'how rise d3'},
          ...
          h('li', null, h('b', null, '?'), h('span', null, 'Outside C: view and close costs 4 points; use costs 18 instead. Either loses Gatekeeper; in P, F or A it also loses Human First.'))
        ),
        h('div', {class:'actions rise d4'},
          h('button', {class:'btn primary', type:'button', 'data-autofocus':'1', onClick:startGame}, 'Start · estimated 9 minutes'),
    ...
    var b = h('button', {class:'est', type:'button', onClick:function(){
      ...
      function ownBox(seen){
        ...
        var go = h('button', {class:'btn primary', type:'button', disabled:'', onClick:function(){
          ...
          trail({k:'A', label:seen ? 'Learner calculation after hints and the full worked example (not assessed)' : 'Learner calculation after hints, before the full worked example (not assessed)', text:text, prov:'learner', cls:'own', pts:0, short:seen ? 'Learner entry after hints and worked example — unassessed' : 'Learner entry after hints, before worked example — unassessed'});
          nextPhase();
        ...
        worked.innerHTML=''; worked.appendChild(h('div', {class:'worked own'}, h('div', {class:'sub-h'}, 'My own calculation'),
          h('p', {class:'note'}, seen
            ? 'The hints and worked example were shown. Your entry is recorded, not assessed for correctness or independence.'
            : 'You have seen hints and feedback. Your entry is recorded, not assessed for correctness or independence.'),
          ta));
      ...
      cont.appendChild(h('div', {class:'actions'},
        ...
        h('span', {class:'hint'}, 'Writing records your reasoning after scaffolding; it does not establish independent work.')));
...
function screenC(){
  ...
  var chosen = null;
  var stageBox = h('div');
  var chat = h('div', {class:'chat'}, h('div', {class:'bar'}, h('span', {class:'led'}), 'AI assistant · phase C · scripted practice case · consultation open'), h('div', {class:'body'}, h('div', {class:'msg bot'}, h('div', {class:'who'}, 'Assistant'), 'Choose a request for this practice exchange.')));
  var body = chat.querySelector('.body');
  var opts = h('div', {class:'opts', role:'group', 'aria-label':'Prompts'});
  var a = attempt();
  var gapText = a.ai ? 'The only number on the table is the AI\'s: '+a.est+'. No learner attempt is on record.' : 'Your selected estimate is '+a.est+'. Identify the boundary assumption you want to check.';
  ...
  var attachCard = a.ai ? null : h('details', {class:'attach panel'},
    h('summary', null, 'Selected estimate: ' + a.est + ' · approach and recorded work'),
    h('div', {class:'q'}, (a.methodShort ? 'Approach: ' + a.methodShort + '.\n' : '')
      + (a.provenance==='learner' ? 'Learner entry (not assessed): ' : 'Worked example I adopted: ') + a.work));
  ...
  var pickBox = h('div', null, h('div', {class:'sub-h'}, 'Which request do you send?'), h('p', {class:'note script-scope'}, a.ai ? 'The reply contains scripted claims about this scenario. No learner attempt is recorded; the earlier output belongs to AI.' : 'The reply critiques the selected estimate and its supplied example route. Typed calculations are saved verbatim but are not read or graded by this script. Practice points follow your selections.'), attachCard, opts, h('div', {class:'actions'}, keyHint()));
  ...
  function choose(p, btn, text){
    ...
    var fbEl = h('div', {class:'feedback '+(p.kind==='bnd'?'':(p.kind==='sub'?'bad':'warn'))}, h('span', {class:'verdict'}, p.kind==='bnd'?'Bounded.':(p.kind==='sub'?'That hands over the task.':'Close, but leaky.')), (a.ai ? 'The earlier setup belongs to AI; no learner attempt is on record. This choice changes only the scope of this request.' : p.fb), h('span', {class:'cls', html:tagFor(p.kind)}));
  ...
  function fixer(p){
    ...
    d.parts.forEach(function(pt, i){
      var b = h('button', {class:'part'+(pt.need?'':' trap'), type:'button', 'aria-pressed':'false', onClick:function(){ on[i]=!on[i]; b.setAttribute('aria-pressed', String(!!on[i])); upd(); }}, h('span', {class:'lbl'}, a.ai && pt.l==='My attempt' ? 'Earlier AI output' : pt.l), tpl(pt.t));
      parts.appendChild(b);
    });
    var sendBtn = h('button', {class:'btn ai', type:'button', disabled:'', onClick:function(){
      ...
      if(needAll && !trapAny){
        box.appendChild(h('div', {class:'feedback'}, h('span', {class:'verdict'}, 'Boundary repaired.'), (a.ai ? 'Earlier AI output identified, gap named, job and boundary set. This does not recover the missing learner attempt. ' : 'Attempt, gap, job, boundary — and no add-on that re-delegates the task. ') + 'Same credit as a bounded request first time. ', h('span', {class:'cls', html:'<b>'+CLS.sup+'</b> — the AI critiques work you made.'})));
...
function screenE(){
  ...
  list.forEach(function(c, i){
    ...
    function returnToC(fu){
      ...
      var compose = h('div', {class:'compose'}, h('div', {class:'lbl'}, 'Your question — edit it if you like; the boundary sentence is fixed'), q, h('div', {class:'fixed'}, fixed), h('div', {class:'row'}, sendBtn, h('span', {class:'hint'}, 'The reply is fixed for this claim; edits are saved, not interpreted. It gives no new number or verdict. Then you may re-judge once.')));
...
function askAI(){
  ...
  modal = h('div', {class:'modal'},
    ...
    h('div', {class:'body'},
      ...
      h('div', {class:'consq', html:'<b>Consequence.</b> '+esc(consq)+' <b>Viewing costs the Gatekeeper badge</b>'+((k==='P' || k==='F' || k==='A') ? ' and the Human First badge' : '')+'. Closing costs 4 points; using costs 18 instead. These are game rules; viewing and rejecting is recorded separately from using the reply.'+(trayAllowed() ? ' To work independently at no cost, use the Evidence tray instead.' : '')})
    ),
    h('div', {class:'foot'},
...
function debriefScreen(){
  ...
  var headline = yes===shown.length ? 'All practice checkpoints met.' : yes+' of '+shown.length+' practice checkpoints met. Here is what the trail records.';
  var clean = !r.given && !st.uncritical && r.promptFinal==='bnd';
  var subline = clean ? 'Nothing was handed over; no verdict rested on the assistant\'s say-so or on convenience.' : (r.given ? 'Some actions incurred practice-point costs. Their use or rejection is recorded below.' : 'Nothing was handed over outright — but look where it leaked.');
  ...
  var lossBox = losses.length ? h('div', {class:'stat rise d4', style:'grid-column:1 / -1'}, h('h3', null, 'Point costs and judgment choices'),
...
/* ---------- Results ---------- */
function dimShort(d){ return d.state==='yes' ? 'met' : (d.state==='no' ? 'not met' : (d.state==='na' ? 'not needed' : (d.id==='attempt' ? 'recorded; independent work not assessed' : (d.id==='constraints' ? 'checks pending' : 'partly')))); }
  ...
  var node = h('div', null,
    h('div', {class:'rank rise'}, confetti,
      ...
      h('div', {class:'debrief', style:'margin-top:18px; text-align:left'},
        ...
        h('div', {class:'stat'}, h('h3', null, 'Practice checkpoints'), h('div', {class:'bignum'}, demo, h('small', null, ' / '+dimsShown.length+' met')), h('div', {class:'sub'}, (notDemo.length ? 'Still open: '+notDemo.map(function(x){ return x.d.label.toLowerCase()+' — '+(x.d.state==='partial' ? 'partly met' : 'not met'); }).join('; ') : 'all applicable practice checkpoints met; independent performance not assessed')+(naDims ? ' · '+naDims+' checkpoint'+(naDims===1?'':'s')+' not applicable to this run, and not counted' : '')))
      ),
